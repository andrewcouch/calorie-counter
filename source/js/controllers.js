...
calCounterApp.controller('calCounterCtrl', ['$scope','localStorageService','$http', 
	function ($scope,lsService,$http) {
		...
		$scope.addFood = function() {
			var cal = calcRelativeCalories($scope.entrytype, $scope.foodcal, $scope.foodamttotal, $scope.foodamt);
		...
		$scope.saveFoodToStored = function(){
			...
			else
			{
				$scope.storedfood_local.push({
					name:$scope.foodname, 
					cal: calcRelativeCalories($scope.entrytype, $scope.foodcal, $scope.foodamttotal, $scope.foodamt),
					unit:$scope.foodunit, 
					amt:$scope.foodamt,	
		...
		$scope.makeFoodLabel = function(food){
			...
			return food.name + " (" + food.amt + " " + unit + ")";
		}
		...
		{
			var found = false;
			var shown_day = $scope.activedate.getFullYear() + "." + $scope.activedate.getMonth() + "." + $scope.activedate.getDate();
